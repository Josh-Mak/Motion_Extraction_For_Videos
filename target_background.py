...
def combine_images(base_image, new_image, threshold, green_image):  # thresh default 50
    ...
    # Composite the green differences onto the new image using the mask
    motion_image = Image.composite(green_image, new_image, mask)
    return motion_image


def process_video_target_background(video_path, threshold=50, compare_first=False, by_interval=False, interval=1, first_to_last_only=False):
